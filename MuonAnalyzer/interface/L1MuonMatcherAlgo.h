...
/**
  ...
  "MuonAnalysis/MuonAssociators/interface/L1MuonMatcherAlgo.h"
...
public:
  explicit L1MuonMatcherAlgoT(const edm::ParameterSet &iConfig,
  ...
  /// Extrapolate a SimTrack to the muon station 2, return an invalid TSOS if it
  /// fails. Requires SimVertices to know where to start from.
  TrajectoryStateOnSurface extrapolate(
      const SimTrack &tk, const edm::SimVertexContainer &vtx) const {
  ...
  /// Find the best match to stage2 L1, and return its index in the vector (and
  /// update deltaR, deltaPhi and propagated TSOS accordingly)
  /// Returns -1 if the match fails
  /// The preselection cut on stage2 L1, if specified in the config, is applied
  /// before the match
  int match(const reco::Track &tk, const std::vector<l1t::Muon> &l1,
  ...
  /// The preselection cut on stage2 L1, if specified in the config, is applied
  /// before the match
  int match(const reco::Candidate &c, const std::vector<l1t::Muon> &l1,
            float &deltaR, float &deltaPhi,
            TrajectoryStateOnSurface &propagated) const {
  ...
  /// update deltaR, deltaPhi and propagated TSOS accordingly)
  /// Returns -1 if the match fails
  /// The preselection cut on stage 2 L1, if specified in the config, is applied
  /// before the match
  int match(const SimTrack &tk, const edm::SimVertexContainer &vtxs,
            const std::vector<l1t::Muon> &l1, float &deltaR, float &deltaPhi,
            TrajectoryStateOnSurface &propagated) const {
    propagated = extrapolate(tk, vtxs);
    return propagated.isValid() ? match(propagated, l1, deltaR, deltaPhi) : -1;
  }

  /// Find the best match to stage 2 L1, and return its index in the vector (and
  /// update deltaR, deltaPhi accordingly)
  /// Returns -1 if the match fails
  /// The preselection cut on stage 2 L1, if specified in the config, is applied
  /// before the match
  int match(TrajectoryStateOnSurface &propagated,
            const std::vector<l1t::Muon> &l1, float &deltaR,
            float &deltaPhi) const;

  // Generic matching

  /// Find the best match to L1, and return its index in the vector (and update
  /// deltaR, deltaPhi and propagated TSOS accordingly)
  /// Returns -1 if the match fails
  ...
  /// Find the best match to L1, and return its index in the vector (and update
  /// deltaR, deltaPhi accordingly)
  /// Returns -1 if the match fails
  /// Only the objects passing the selector will be allowed for the match.
  /// The selector defaults to an AnySelector (CommonTools/Utils) which just
  /// accepts everything
  template <typename Collection, typename Selector>
  ...
  /// Add this offset to the L1 phi before doing the match, to correct for
  /// different scales in L1 vs offline
...
private:
  ...
  //  int genericQuality(l1extra::L1MuonParticle const &cand) const { return
  // cand.gmtMuonCand().rank(); }
...
int L1MuonMatcherAlgoT<Tr>::matchGeneric(TrajectoryStateOnSurface &propagated,
                                         ...
                                         const Selector &sel, float &deltaR,
                                         float &deltaPhi) const {
  ...
  for (int i = 0, n = l1s.size(); i < n; ++i) {
    ...
    if (sel(l1)) {
      ...
      double thisDeltaPhi =
          ::deltaPhi(double(pos.phi()), l1.phiAtVtx() + l1PhiOffset_);
      double thisDeltaEta = pos.eta() - l1.etaAtVtx();
      double thisDeltaR2 =
          ::deltaR2(double(pos.eta()), double(pos.phi()), l1.etaAtVtx(),
                    l1.phiAtVtx() + l1PhiOffset_);

      int thisQual = genericQuality<obj>(l1);
      double thisPt = l1.pt();
      if ((fabs(thisDeltaPhi) < deltaPhi_) &&
          (fabs(thisDeltaEta) < deltaEta_) &&
        ...
        switch (sortBy_) {
          ...
          case SortByPt:
            ...
            break;
          // Quality is an int, adding sorting by pT in case of identical
          // qualities
          case SortByQual:
            betterMatch = (thisQual > maxQual ||
                           ((thisQual == maxQual) && (thisPt > maxPt)));
            break;
        }
...
L1MuonMatcherAlgoT<Tr>::L1MuonMatcherAlgoT(const edm::ParameterSet &iConfig,
    ...
    : propSetup_(iConfig, iCollector),
      useStage2L1_(iConfig.existsAs<bool>("useStage2L1")
                       ? iConfig.getParameter<bool>("useStage2L1")
                       : false),
      ...
      deltaPhi_(iConfig.existsAs<double>("maxDeltaPhi")
                    ...
                    : 10),
      deltaEta_(iConfig.existsAs<double>("maxDeltaEta")
                    ? iConfig.getParameter<double>("maxDeltaEta")
                    : 10),
      l1PhiOffset_(iConfig.existsAs<double>("l1PhiOffset")
                       ? iConfig.getParameter<double>("l1PhiOffset")
                       : 0) {
  bool reqQual = iConfig.existsAs<bool>("sortByQual") &&
                 iConfig.getParameter<bool>("sortByQual");
  bool reqPhi = iConfig.existsAs<bool>("sortByDeltaPhi") &&
                iConfig.getParameter<bool>("sortByDeltaPhi");
  bool reqEta = iConfig.existsAs<bool>("sortByDeltaEta") &&
                iConfig.getParameter<bool>("sortByDeltaEta");
  bool reqPt = iConfig.existsAs<bool>("sortByPt") &&
               iConfig.getParameter<bool>("sortByPt");
  std::string sortBy = iConfig.existsAs<std::string>("sortBy")
                           ? iConfig.getParameter<std::string>("sortBy")
                           : "";
  if (reqPhi + reqEta + reqPt + reqQual > 1)
    throw cms::Exception("Configuration") << "L1MuonMatcherAlgoT: Can't set "
                                             "more than one 'sortBy<XXX>' "
                                             "parameter to True.\n";
  if (sortBy == "deltaPhi") {
    if (reqEta || reqPt || reqQual)
      throw cms::Exception("Configuration")
          << "L1MuonMatcherAlgoT: Can't set sortBy = 'deltaPhi' and set also "
             "another "
  ...
  if (sortBy == "deltaEta") {
    if (reqPhi || reqPt || reqQual)
      throw cms::Exception("Configuration")
          << "L1MuonMatcherAlgoT: Can't set sortBy = 'deltaEta' and set also "
             "another "
             "'sortBy<XXX>' parameter to True.\n";
    else
      reqEta = true;
  ...
  if (sortBy == "pt") {
    if (reqPhi || reqEta || reqQual)
      throw cms::Exception("Configuration")
          << "L1MuonMatcherAlgoT: Can't set sortBy = 'pt' and set also another "
             "'sortBy<XXX>' parameter to True.\n";
    else
      reqPt = true;
  }
  if (sortBy == "quality") {
    if (reqPhi || reqEta || reqPt)
      throw cms::Exception("Configuration")
          << "L1MuonMatcherAlgoT: Can't set sortBy = 'quality' and set also "
             "another 'sortBy<XXX>' parameter to True.\n";
    else
      reqQual = true;
  }
  if (sortBy == "deltaR") {
    if (reqPhi || reqEta || reqPt || reqQual)
      throw cms::Exception("Configuration")
          << "L1MuonMatcherAlgoT: Can't set sortBy = 'deltaR' and set also "
             "another 'sortBy<XXX>' parameter to True.\n";
  }
  // so, if we're here there's no ambiguity in what the user may want. either
  // everything is false, or exactly one req is true.
  if (reqEta)

...
/**
  \class    pat::L1MuonMatcher L1MuonMatcher.h
  "MuonAnalysis/MuonAnalyzer/interface/L1MuonMatcher.h"
...
#include "MuonAnalysis/MuonAnalyzer/interface/L1MuonMatcherAlgo.h"
...
private:
  ...
  void storeExtraInfo(edm::Event &iEvent, const Hand &handle,
...
pat::L1MuonMatcher::L1MuonMatcher(const edm::ParameterSet &iConfig)
    : matcher_(iConfig, consumesCollector()),
      recoToken_(consumes<edm::View<reco::Candidate> >(
  ...
  if (useStage2L1_) {
    l1tToken_ = consumes<l1t::MuonBxCollection>(
        iConfig.getParameter<edm::InputTag>("matched"));
  } else {
    l1Token_ = consumes<std::vector<l1extra::L1MuonParticle> >(
        iConfig.getParameter<edm::InputTag>("matched"));
  }
  ...
  produces<PATTriggerAssociation>(
      "propagatedReco");              // asso reco to propagated reco
...
void pat::L1MuonMatcher::produce(edm::Event &iEvent,
  ...
  vector<int> quality(reco->size(), 0), bx(reco->size(), -999),
  ...
  for (int i = 0, n = reco->size(); i < n; ++i) {
    ...
    int match = matcher_.match(mu, l1ts, deltaRs[i], deltaPhis[i], propagated);
    if (propagated.isValid()) {
      GlobalPoint pos = propagated.globalPosition();
      propMatches[i] = propOut->size();
      propOut->push_back(PATPrimitive(math::PtEtaPhiMLorentzVector(
          mu.pt(), pos.eta(), pos.phi(), mu.mass())));
    ...
    if (match != -1) {
      ...
      if (useStage2L1_) {
        ...
        bx[i] = l1tBX->getFirstBX() +
                (std::upper_bound(bxIdxs.begin(), bxIdxs.end(), match) -
                 bxIdxs.begin());
  ...
  OrphanHandle<PATPrimitiveCollection> l1Done =
      iEvent.put(std::move(l1Out), "l1muons");
  OrphanHandle<PATPrimitiveCollection> propDone =
      iEvent.put(std::move(propOut), "propagatedReco");

  unique_ptr<PATTriggerAssociation> propAss(
      new PATTriggerAssociation(propDone));
...
template <typename Hand, typename T>
void pat::L1MuonMatcher::storeExtraInfo(edm::Event &iEvent, const Hand &handle,
                                        const std::vector<T> &values,
                                        const std::string &label) const {
  using namespace edm;
  using namespace std;
